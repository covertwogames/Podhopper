...
public class MediaLibrarySessionCallback implements MediaLibraryService.MediaLibrarySession.Callback {
    ...
    private List<MediaItem> enrichMediaItems(List<MediaItem> mediaItems) {
        ...
        for (MediaItem item : mediaItems) {
            FeedMedia media = resolveRequestedMedia(item);
            if (media != null) {
    ...
    @WorkerThread
    private FeedMedia resolveRequestedMedia(MediaItem item) {
        try {
            return DBReader.getFeedMedia(Long.parseLong(item.mediaId));
            ...
            return resolveVoiceQuery(query.trim());
        }
    }

    @WorkerThread
    private FeedMedia resolveVoiceQuery(String query) {
        Log.d(TAG, "Resolving voice query: " + query);
        if (query.isEmpty()) {
            return DBReader.getFeedMedia(PlaybackPreferences.getCurrentlyPlayingFeedMediaId());
        }
        Feed bestFeed = findBestFeedMatch(query);
        if (bestFeed != null) {
            FeedMedia episode = selectEpisodeToPlay(bestFeed);
            if (episode != null) {
                Log.d(TAG, "Voice query matched feed: " + bestFeed.getTitle());
                return episode;
            }
        }
        for (FeedItem result : DBReader.searchFeedItems(0, query, FeedItemFilter.unfiltered())) {
            if (result.getMedia() != null) {
                Log.d(TAG, "Voice query matched episode: " + result.getTitle());
                return result.getMedia();
            }
        }
        Log.d(TAG, "Voice query matched nothing: " + query);
        return null;
    }

    @WorkerThread
    private Feed findBestFeedMatch(String query) {
        String normalizedQuery = query.toLowerCase(Locale.getDefault());
        Feed bestFeed = null;
        int bestScore = 0;
        for (Feed feed : DBReader.searchFeeds(query, FeedItemFilter.unfiltered())) {
            if (feed.getTitle() == null) {
                continue;
            }
            String title = feed.getTitle().toLowerCase(Locale.getDefault());
            int score;
            if (title.equals(normalizedQuery)) {
                score = 4;
            } else if (title.startsWith(normalizedQuery) || normalizedQuery.startsWith(title)) {
                score = 3;
    ...
    @WorkerThread
    private FeedMedia selectEpisodeToPlay(Feed feed) {
        List<FeedItem> episodes = DBReader.getFeedItemList(
                feed, FeedItemFilter.unfiltered(), SortOrder.DATE_NEW_OLD, 0, 50);
        FeedMedia newest = null;
