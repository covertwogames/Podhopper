...
private fun BackgroundRefreshRow(
    ...
    SettingRadioDialogRow(
        ...
        options = PodcastRefreshFrequency.all,
        savedOption = state.frequency,
        optionToLocalisedString = { context.getString(it.stringRes) },
        onSave = { state.onFrequencyChange(it) },
        modifier = modifier,
        secondaryText = stringResource(state.frequency.stringRes),
    )
}
        ...
        StorageSettingsView(
            state = StorageSettingsViewModel.State(
                ...
                backgroundRefreshState = StorageSettingsViewModel.State.BackgroundRefreshState(
                    frequency = PodcastRefreshFrequency.Hours1,
                    onFrequencyChange = {},
                ),
