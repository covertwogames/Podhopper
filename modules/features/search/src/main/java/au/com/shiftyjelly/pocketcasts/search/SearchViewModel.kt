...
class SearchViewModel @Inject constructor(
    ...
    fun updateSearchQuery(query: String, immediate: Boolean = false) {
        ...
        // PodHopper: a pasted RSS feed URL subscribes directly via the client-side feed engine (no Pocket Casts server).
        val trimmedQuery = query.trim()
        if ((trimmedQuery.startsWith("http://", ignoreCase = true) || trimmedQuery.startsWith("https://", ignoreCase = true)) &&
            trimmedQuery.contains(".") && trimmedQuery.length > 12
        ) {
            viewModelScope.launch { podcastManager.subscribeToFeedUrl(trimmedQuery) }
            return
        }
    ...
    /**
     * Open an iTunes search result instantly: insert a lightweight NOT subscribed podcast from the
     * result's own metadata and start loading its episodes in the background, returning the uuid so
     * the caller can open the real podcast page right away. Returns null only if the result has no
     * feed url.
     */
    suspend fun addFeedStub(item: ImprovedSearchResultItem.PodcastItem): String? {
        val feedUrl = item.feedUrl
        if (feedUrl.isNullOrBlank()) {
            return null
        }
