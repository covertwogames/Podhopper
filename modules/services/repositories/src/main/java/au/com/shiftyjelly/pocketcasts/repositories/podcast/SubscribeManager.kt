...
class SubscribeManager @Inject constructor(
    ...
    /**
     * PodHopper client-side subscribe: fetch and parse an RSS feed directly (no Pocket Casts
     * server) and store it as a normal subscribed podcast with its episodes. This runs blocking,
     * so it must be called off the main thread.
     */
    fun subscribeToFeedUrlBlocking(feedUrl: String) {
