...
import au.com.shiftyjelly.pocketcasts.models.to.PodcastRefreshFrequency
...
class StorageSettingsViewModel @Inject constructor(
    ...
    private fun initState() = State(
        ...
        backgroundRefreshState = State.BackgroundRefreshState(
            frequency = settings.podcastRefreshFrequency.value,
            onFrequencyChange = { onRefreshFrequencyChange(it) },
        ),
    ...
    private fun onRefreshFrequencyChange(frequency: PodcastRefreshFrequency) {
        settings.podcastRefreshFrequency.set(frequency, updateModifiedAt = true)
        // keep the existing on/off flag in sync for settings that reference it (e.g. sync on metered)
        settings.backgroundRefreshPodcasts.set(frequency.isOn, updateModifiedAt = true)
    ...
    data class State(
        ...
        data class BackgroundRefreshState(
            val frequency: PodcastRefreshFrequency,
            val onFrequencyChange: (PodcastRefreshFrequency) -> Unit,
        )
